...
XSPECFile = os.path.join(basedir, 'sixte_files/2comp/A1689.xcm')
IMGFile = os.path.join(basedir, 'data/csm_sb_cropR500.fits')
#Lx_obs = 9.49665e+44
#Flux = Lx_obs / ( 4. * np.pi * (cosmo.luminosity_distance(z=0.1842).to(u.cm)**2))
Flux = 9.92495883e-12
p2simput = os.path.join(basedir, 'sixte_files/2comp/A1689.simput')
...
# Now do 2 component, 1 pointing in the center with resolve, xtend
RA_pnt = 197.87233
DEC_pnt = -1.341345
texp = 2e5 #200ks
#Generate events
p2evt_resolve = os.path.join(basedir, 'sixte_files/2comp/A1689_evts_resolve.fits')
A1689.runSIXTE_resolve(p2evt_resolve, RA_pnt, DEC_pnt, texp)

p2evt_xtend = os.path.join(basedir, 'sixte_files/2comp/A1689_evts_xtend.fits')
A1689.runSIXTE_xtend(p2evt_xtend, RA_pnt, DEC_pnt, texp)

#Make images
p2img = os.path.join(basedir,'sixte_files/2comp/A1689_IMG_resolve.fits')
A1689.createIMAGE_resolve(p2evt_resolve, p2img, RA_pnt, DEC_pnt)

p2img = os.path.join(basedir,'sixte_files/2comp/A1689_IMG_xtend.fits')
A1689.createIMAGE_xtend(p2evt_xtend, p2img, RA_pnt, DEC_pnt)

#Extract and fit resolve spectrum
p2spec = os.path.join(basedir, 'sixte_files/2comp/A1689_SPEC_resolve.pha')
A1689.extractSPECTRUM_resolve(p2evt_resolve, p2spec)

p2out = os.path.join(basedir, 'sixte_files/2comp/A1689_spec_result.txt')
spectral_fitting.fit_Tbabs_bapec_bapec(p2spec, p2out, Emin=2.0, Emax=10, nH=0.0167, kT=8.8, z=0.184, vel=180)